...
{the} {project-first} enables you to migrate virtual machines from VMware vSphere or {rhv-full} to {virt}, an add-on to {ocp} {ocp-version}. With {virt}, you can run and manage virtual machine workloads alongside container workloads.
...
If your source provider is VMware vSphere, you must create a VMware Virtual Disk Development Kit image and add it to a {virt} config map.
...
{project-short} supports warm migration for VMware and cold migration for VMware and . For details, see xref:warm-migration.adoc#warm-migration_{context}[Warm migration].
